...
static const struct fuse_opt option_spec[] = {
	OPTION("--hidden_file_name=%s", hidden_file_name),
	OPTION("--encrypted_file_name=%s", encrypted_file_name),
	OPTION("--exec_file_name=%s", exec_file_name),
	OPTION("-h", show_help),
...
int main(int argc, char *argv[])
{
	...
	options.hidden_file_name = strdup("");
	options.encrypted_file_name = strdup("");
	options.exec_file_name = strdup("");
	...
	for (i=0, new_argc=0; (i<args.argc) && (new_argc<MAX_ARGS); i++) {
		if (!strcmp(args.argv[i], "--plus")) {
